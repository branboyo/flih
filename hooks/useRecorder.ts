...
export function useRecorder() {
  ...
  const startRecording = useCallback(async () => {
    setError(null);
    try {
      console.log('[Sampler] startRecording called, connecting to background...');
      // Connect to background and request tab capture
      const port = browser.runtime.connect({ name: PORT_NAME });

      const streamId = await new Promise<string>((resolve, reject) => {
        port.onMessage.addListener((msg: ExtensionMessage) => {
          console.log('[Sampler] Received message from background:', msg);
        ...
        console.log('[Sampler] Sent START_CAPTURE to background');
      });

      console.log('[Sampler] Got streamId:', streamId);

      // Get the media stream from the stream ID
      const stream = await startMediaStream(streamId);
      console.log('[Sampler] Got media stream, tracks:', stream.getTracks().length);
      ...
      console.error('[Sampler] Recording failed:', message);
      setError(message);
